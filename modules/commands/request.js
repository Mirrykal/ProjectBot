...
api.changeNickname(`${(!global.config.BOTNAME) ? "Rem" : global.config.BOTNAME}`, handleReply.pending[singleIndex - 1].threadID, api.getCurrentUserID());
...
module.exports.run = async function({ api, event, args, permission, handleReply }) {
    ...
    case "-u":
    case "المستخدمين": {
    const permission = ["61561400245668"];
   ...
   const permission = ["61561400245668"];
    ...
    case "-a":
    case "الكل": {
    const permission = ["61561175994395","61561400245668"];
    if (!permission.includes(event.senderID)) return api.sendMessage("ماعدك صلاحية أستعمال هذا الامر :>", event.threadID, event.messageID);
     const { threadID, messageID } = event;
    const commandName = this.config.name;
    var msg = "", index = 1;
